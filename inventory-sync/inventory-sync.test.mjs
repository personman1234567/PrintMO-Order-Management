...
const inventory = [{ sku: variant.sku, warehouses: [{ warehouseAbbr: 'IL', qty: 8 }, { warehouseAbbr: 'KS', qty: 0 }] }];
...
test('confirmed zero remains zero; missing SKU and warehouse remain unknown', () => {
  assert.equal(makePlan({ ...base, inventory: [{ sku: variant.sku, warehouses: [{ warehouseAbbr: 'IL', qty: 0 }, { warehouseAbbr: 'KS', qty: 0 }] }] }).rows[0].supplierAvailable, 0);
...
test('malformed, negative, duplicate and unexpected supplier data fail closed', () => {
  for (const payload of [null, {}, [inventory[0], inventory[0]], [{...inventory[0],sku:'OTHER'}],
    [{sku:variant.sku, warehouses:[{warehouseAbbr:'IL',qty:-1}]}],
...
test('gateway enforces auth and read method before upstream requests', async () => {
  ...
  const deps={fetchImpl:async()=>{calls++;return response(inventory);}};
