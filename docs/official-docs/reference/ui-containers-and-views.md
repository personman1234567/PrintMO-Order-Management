...
- **Displayed Data & Topology**:
  ...
  - **Always-visible Production Pane (~39% desktop)** (`#detail-left-pane`):
    - **Order Artwork** (`#detail-mockups-strip`): At desktop Shopify-board widths, the selected preview (`#detail-mockup-feature`, `#detail-mockup-main`) derives its frame from the image's intrinsic aspect ratio and caps it to the production pane or a bounded working height. This preserves square, 16:9, and uncommon horizontal artwork without cropping or a misleading empty stage; the image-led frame is followed by a horizontal thumbnail track (`#detail-mockups-track`) and source-aware item context (`#detail-mockup-context`). The context aggregates every garment-size quantity in the selected artwork's product-and-color scope, so a mockup linked to one size represents the other sizes of that same garment color too; unlinked manual artwork is explicitly labeled as order-level rather than assigned to a garment. Upload and paste controls remain available. Selecting a thumbnail updates the large preview; the large preview opens the asset viewer.
    - **Production Progress** (`#detail-production-card`): Progress bar and printed-piece count, immediately responsive `+1 Print`, custom quantity, and visible save feedback. Rapid increments keep one mutation in flight and coalesce to the latest desired count. The custom dialog validates an absolute printed total, provides stepper and `Mark all printed` actions, and reaching the garment total atomically persists the count with canonical `completed` stage.
  ...
  - **Tabbed Workspace (~61% desktop)** (`#detail-right-pane`):
    - **Accessible Tabs** (`#detail-tabs-header`): Action Blue active line, click navigation, arrow-key navigation, and synchronized `tab`/`tabpanel` ARIA state.
      1. `Overview` (`#tab-overview`, default): Current production stage, a navigation-only “next place to look,” material and print progress, fulfillment state, recorded production-attention reasons, current-detail freshness, garment/print counts, order total, and received time. Its actions activate the existing Production, Items, Fulfillment, or Activity workspace; they do not mutate order state or claim artwork approval, formal release, or quality acceptance.
      2. `Items & financials` (`#tab-items`): Presentation-consolidated line items, custom attributes, discount, order total, and a SKU-gated garment accounting control. Selecting `Receive n/n` opens the canonical supplier-batch receiving record at the matching SKU line; increment, typed quantity, and All actions remain batch-authoritative and retain oldest-first order allocation.
      3. `Production` (`#tab-production`): Customer checkout note banner and the full-width Design Files workspace (`#detail-design-panel`), grouped into `Front prints`, `Back prints`, and `Extras`. Current Shopify orders expose a placement selector and multi-file Add design control for SVG, PNG, JPG, and WebP artwork. Successful uploads refresh the canonical detail in place. Only manually uploaded tiles expose Remove; source-managed Designer Studio files remain download-only.
...
- **Current reliability and semantic boundaries**:
  ...
  - Inline desktop note drafts are keyed by provider-aware immutable order identity. Close, backdrop, and the editor's first Escape preserve that order's draft; explicit Cancel discards it; save clears it. Opening another order never exposes or saves the previous order's draft.
  - Shopify/Etsy candidate customer names are visibly source-managed and do not expose the legacy name-edit mutation. Legacy writable records retain the existing editor.
  - Dialog wrapping excludes hidden, inert, negative-tabindex, and zero-geometry controls; the inline notes editor owns its first Escape. Closed detail content leaves layout, the clipped Upload input transfers visible focus to its label, and programmatic tab scrolling respects reduced motion.
  - These repairs are source- and automated-contract verified. Authenticated keyboard, screen-reader, populated-detail, and mobile geometry acceptance remains tracked in the linked Digital Traveler plan.
...
- **Displayed Data**: Textarea element (`#notes-input`) pre-populated with `order.notes`, an inline Saving/failure status that preserves input and offers retry, and `Confirm` / `Cancel` buttons.
