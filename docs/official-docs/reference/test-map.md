...
| Live Synthetic Etsy Pilot | Open `#ETSY-TEST-001` in the embedded production app, inspect its card/detail, move and refresh it if acceptance testing is intended, then use the exact cleanup control when finished | One orange-rail card shows separate `Etsy` and `TEST` text badges; two garment lines and provider detail load; stage changes use revisioned D1 state; artwork upload and S&S batching remain unavailable; cleanup targets only `etsy-synthetic:<shop-id>:synthetic-pilot-1` |
| Etsy `order.paid` Webhook | Follow [Etsy webhook setup and verification](../runbooks/etsy-webhook-setup-and-verification.md), send one Portal test, run the invalid-signature probe, and inspect the authenticated webhook status | Portal sees `2xx`; invalid signature returns `401`; status reports the secret configured, shadow-only mode, no customer data, and a durable delivery outcome; one real eligible receipt reaches hidden shadow exactly once before enrollment is considered |
