...
| Shopify Live Rich Detail — Production Scope Gate | With fulfillment-order read scopes released and approved, open a recent order from **Shopify live** | Detail identifies Shopify as source and renders all sections. Current 2026-07-22 production state fails this prerequisite with `ACCESS_DENIED` at `order.fulfillmentOrders`, surfaced misleadingly as `404 - [object Object]`; do not mark the live test passed until the scope gate is resolved. |
