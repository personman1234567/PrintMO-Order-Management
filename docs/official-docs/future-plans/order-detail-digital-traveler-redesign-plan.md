...
- **Next safe action**: Release the owner-approved manual Target date slice (Worker first, then Pages) and verify it on an active order. The optional date is an internal shop finish-by day in America/Chicago, editable in Overview and visible on cards. Its implementation uses the existing canonical production update contract; see [Manual target dates](../workflows/order-ingestion-kanban.md#manual-target-dates). Broader next-action/owner/blocker semantics still need shop validation.
- **Remaining blockers**: Overview v1, print-progress reliability, and mobile Detail navigation have no remaining release blocker. The local navigation fixture passed 320px, 393px, and desktop interaction checks, but final native-app acceptance remains owner-observed. The manual Target date slice has approved semantics and local verification; its remaining step is coordinated Worker/Pages deployment. Broader workflow slices still require exact shop labels, transition authority, and customer-promise semantics.
- **Owner / external actions**: The internal Target date meaning and manual editing were approved on 2026-09-14; local implementation is pending deployment. Before broader workflow work, confirm who owns a next action, which customer promise is authoritative, and what qualifies as a recorded blocker.
