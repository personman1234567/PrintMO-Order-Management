...
The Worker contains a guarded, single-variant Shopify quantity writer for a future pilot. Production remains `INVENTORY_SYNC_MODE=disabled`, with no public route or cron, and the deployed Shopify credential is read-only. The writer cannot run with production settings: it additionally requires a verified supplier-feed commitment model, a positive maximum write delta, a single variant, protected locations, `write_inventory`, and an online S&S Supplier location. Shopify's S&S Supplier location was staged on 2026-09-22 with online fulfillment disabled and no active pilot inventory. On 2026-09-25 a separate, manual Shopify stockout-only pilot enabled tracking for exactly one physically sold-out Tultex 246 variant, leaving its existing zero HQ balance unchanged; Shopify and public product JSON both report it unavailable. This proves a narrow block, not automatic supplier sync or restock. Dry-run capacity before commitments remains an observation, not a sellable quantity; its rows still report `writeReady: false` and `proposedQuantity: null`.

## Current Continuation State

- **Current state**: Gateway PRs #1–#3 are merged. Render manual deploy `dep-daq0hmd9fdbs73eofpj0` is Live at `1014823`, with its inventory-only key configured and Auto-Deploy Off. The gateway joins S&S inventory quantities to product warehouse metadata and forwards only the boolean dropship flag. The dedicated Shopify app, Print-MO Inventory Observation, has only `read_products` and `read_inventory`; its client credentials and the matching gateway key remain Cloudflare secrets. A 2026-09-23 Cloudflare **remote preview** ran the 25 Tultex variants with all non-dropship S&S warehouses selected: `inStock=21`, `outOfStock=4`, `unknown=0`, `blocked=25`, `writes=0`. The previously untracked Shopify stockout variant remained buyable at that time. On 2026-09-25, the guarded writer was built, 26 inventory tests and the Wrangler dry-run build passed, and Worker version `fad82450-90ea-428b-a188-08091aa45f6e` was deployed. Its binding remains `disabled`, its protected-location list is empty, no source-semantics policy or write-delta limit is configured, and it has no public route or cron. A fresh direct S&S read then confirmed physical warehouse quantity zero for Tultex 246 Vintage White/Mellow Yellow / Small, SKU `B30459123`, variant `gid://shopify/ProductVariant/46257399660792`; DS/dropship was excluded. Shopify had one exact SKU match, `DENY` policy, one HQ level with available zero, no S&S level, and `availableForSale=true` while untracked. A one-variant `productVariantsBulkUpdate` set only its inventory item to `tracked=true`. Readback showed `availableForSale=false`, and public product JSON showed `available=false`; the neighboring Medium variant stayed untracked and available. No quantity, location, Shopify app scope, or Worker configuration was changed.
- **Next safe action**: Keep the one stockout pilot under observation and decide the safe reopen rule before adding automatic supplier writes. Recheck exact S&S physical stock before any reopening; this variant will remain blocked if S&S restocks because the Worker is disabled. Resolve HQ online-stock participation, supplier fulfillment/delivery routing, app permissions, and commitment/shared-SKU accounting before a positive-quantity pilot. A five-minute cadence remains a proposal, not a configured schedule.
- **Remaining blockers**: Production observation is intentionally disabled. Physical S&S stock is classified without dropship rows, but commitment semantics, local-stock policy, shipping eligibility and shared-SKU allocation are not settled. A positive physical-warehouse response is not automatically sellable quantity. The dedicated Shopify app still has only read scopes; the existing order-capable credential was not copied to the inventory Worker. The S&S location remains offline; only the one stockout pilot variant is now tracked, with the other 24 pilot variants unchanged.
...
- [ ] Verify supplier quantity semantics, full shared-SKU mapping, required Shopify scopes and location/tracking settings, then conduct a reviewed live single-variant stockout/reopen pilot.
...
Worker non-secret settings: `SHOPIFY_SHOP_DOMAIN`, `INVENTORY_SYNC_MODE` (`disabled`, `dry-run`, or guarded `pilot-write`), `PILOT_VARIANT_IDS` (JSON ID array), `SS_WAREHOUSES` (JSON abbreviation array or `["*"]` for all physical S&S warehouses), `SS_SAFETY_BUFFER` (nonnegative integer string), `SUPPLIER_LOCATION_ID` (optional for observation), `PROTECTED_LOCATION_IDS` (JSON ID array), and `SUPPLIER_INVENTORY_URL`. `pilot-write` additionally requires the exact policy marker `SUPPLIER_FEED_SEMANTICS=gross-before-shopify-commitments`, a positive integer `INVENTORY_MAX_WRITE_DELTA`, exactly one variant ID, at least one protected location, and the hardcoded Print-MO shop and S&S location. These write settings are intentionally absent from production. Do not set the policy marker until S&S feed semantics are established from supplier/order evidence.
...
The dry-run gate evidence remains non-executable. It reads `availableForSale`, `sellableOnlineQuantity`, and all reported inventory levels (rejecting pagination). If supplier stock after the safety buffer is zero, it identifies `ZERO_SUPPLIER_AVAILABLE` as the candidate block; if positive, reopening still requires commitment/reservation accounting. Positive stock at another active location may defeat a supplier-only stockout. The current read-only credential cannot retrieve location online-fulfillment settings. The staged S&S Supplier location was last verified offline, and all 25 pilot variants remain untracked, so the disabled production Worker does not establish a working storefront gate.

The guarded writer reads the exact variant, active S&S location, granted `write_inventory` scope, and exact SKU uniqueness before a write. It rejects untracked variants, Shopify oversell policy, unknown or positive stock at any other active location, stale S&S data, non-physical/dropship inventory, unexpected Shopify inventory states, and changes above the configured delta. Under the explicitly verified gross-before-commitments feed model, it targets `max(0, physical S&S quantity - buffer - Shopify committed)` at the S&S location only. The mutation compares Shopify's prior `available` quantity and uses a stable idempotency key across its bounded HTTP retry. It does not change tracking, location settings, checkout, or Shopify app scopes. If the supplier feed already nets out Shopify commitments, this formula would subtract twice; that source-model gate must be resolved before enabling it.
