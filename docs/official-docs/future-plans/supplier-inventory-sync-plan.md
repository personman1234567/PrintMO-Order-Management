...
The Worker now contains a guarded, single-variant Shopify quantity writer for a future pilot. Production remains `INVENTORY_SYNC_MODE=disabled`, with no public route or cron, and the deployed Shopify credential is read-only. The writer cannot run with production settings: it additionally requires a verified supplier-feed commitment model, a positive maximum write delta, a single variant, protected locations, `write_inventory`, and an online S&S Supplier location. Shopify's S&S Supplier location was staged on 2026-09-22 with online fulfillment disabled and no active pilot inventory. Dry-run capacity before commitments remains an observation, not a sellable quantity; its rows still report `writeReady: false` and `proposedQuantity: null`.

## Current Continuation State

- **Current state**: Gateway PRs #1–#3 are merged. Render manual deploy `dep-daq0hmd9fdbs73eofpj0` is Live at `1014823`, with its inventory-only key configured and Auto-Deploy Off. The gateway joins S&S inventory quantities to product warehouse metadata and forwards only the boolean dropship flag. The dedicated Shopify app, Print-MO Inventory Observation, has only `read_products` and `read_inventory`; its client credentials and the matching gateway key remain Cloudflare secrets. A 2026-09-23 Cloudflare **remote preview** ran the 25 Tultex variants with all non-dropship S&S warehouses selected: `inStock=21`, `outOfStock=4`, `unknown=0`, `blocked=25`, `writes=0`. A subsequent local read-only 25-variant Shopify/S&S check selected one out-of-stock pilot variant: Shopify reported `tracked=false`, `availableForSale=true`, `sellableOnlineQuantity=0`, no active S&S Supplier level and no positive stock at another active level; the public product JSON also reported that exact variant as `available=true`. The gate plan stayed blocked with `writes=0`. These are physical-warehouse observations before commitments, not Shopify sellable quantities. On 2026-09-25, the guarded writer was built, 26 inventory tests and the Wrangler dry-run build passed, and Worker version `fad82450-90ea-428b-a188-08091aa45f6e` was deployed. Its binding remains `disabled`, its protected-location list is empty, no source-semantics policy or write-delta limit is configured, and it has no public route or cron. No Shopify write was attempted.
- **Next safe action**: Validate the gate-evidence and writer-prerequisite reads in a hosted Cloudflare preview with the dedicated app, then resolve HQ online-stock participation, supplier fulfillment/delivery routing, the supplier feed's commitment semantics, and shared-SKU accounting before a live single-variant trial. A five-minute read-only observation cadence remains a proposal, not a configured schedule. The earlier KS-only result was diagnostic and is superseded for warehouse-scope observation by the all-physical-warehouse run.
- **Remaining blockers**: Production observation is intentionally disabled. Physical S&S stock is now classified without dropship rows, but commitment semantics, local-stock policy, shipping eligibility and shared-SKU allocation are not settled. A positive physical-warehouse response is not automatically sellable quantity. The dedicated Shopify app still has only read scopes; the existing order-capable credential was not copied to the inventory Worker. The S&S location remains offline and the candidate variants remain untracked.
