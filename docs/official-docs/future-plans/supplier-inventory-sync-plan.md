...
- **Current state**: Gateway PRs #1 and #2 are merged; Render deploy `dep-dapvctvlk1mc73d1rb8g` is Live at `0c6acf5` with its inventory-only key configured and Auto-Deploy Off. The dedicated Shopify app, Print-MO Inventory Observation, is installed with only `read_products` and `read_inventory`; its client credentials and the matching gateway key are stored as Cloudflare Worker secrets, with no credential values in source. A bounded authenticated gateway read returned one requested item. A 2026-09-23 Cloudflare **remote preview** executed the scheduled handler once in dry-run mode with KS as a diagnostic-only warehouse: `variants=25`, `blocked=25`, `writes=0`. The first preview invocation exposed a Workers runtime incompatibility with `redirect: 'error'`; `redirect: 'manual'` now rejects redirects without forwarding credentials and the successful preview used that fix. Production Worker version `0cce3ed5-5a34-4cfc-a7f6-ef2ffe9f356e` has the fix but remains disabled, with empty warehouse selection, no public route, and no cron. The source fix and pilot setup are pushed to the remote `Shopify-Sync` branch at `2e84eb2`, not merged to `main`. The earlier local KS diagnostic found quantities for 19 SKUs and no KS coverage for six; those counts do not set saleable inventory.
- **Next safe action**: Decide supplier warehouse eligibility and quantity meaning, commitment/reservation accounting, HQ physical-stock treatment, and fulfillment routing. Then add persisted status/alerts and an approved read-only observation cadence before considering any inventory writer. Keep KS diagnostic-only until warehouse policy is verified.
- **Remaining blockers**: The hosted connectivity check is complete, but production observation is intentionally disabled. Supplier warehouse eligibility/quantity semantics, commitment accounting, local-stock policy and fulfillment behavior are not settled. A successful supplier response or KS diagnostic is not evidence of sellable quantity. The existing order-capable Shopify credential was not copied to the inventory Worker.
- **Owner / external actions**: Settle eligible warehouses and local-stock policy before any sellable quantity is derived. Configure supplier location/fulfillment and reviewed pilot writes after the commerce model is ready. No need to paste secrets into chat.
...
- [ ] Decide warehouse policy, persist status, and add an approved read-only observation schedule and alerts.
