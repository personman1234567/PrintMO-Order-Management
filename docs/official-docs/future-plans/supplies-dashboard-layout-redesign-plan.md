...
- **Current state**: Shopify desktop now uses a tabbed Supplies workspace in the existing middle panel. Exactly one body is visible at a time: `Build Order`, `In S&S Cart` (the default), or `Ordered`. `Build Order` is activated after a short hover when a Pipeline card is dragged over the panel. Desktop Shopify work queues retain a two-column card grid even when a queue has one card. The existing cart/ordered state model and Worker mutations are unchanged. Ready to Print now receives the full right panel height. Shopify is the initial board; Legacy Redis and the mobile layout remain switchable and otherwise unchanged.
- **Next safe action**: Perform Shopify desktop acceptance at widths above 900px, then decide whether to redesign the mobile Supplies navigation and add the Ready to Print sort/filter toolbar.
