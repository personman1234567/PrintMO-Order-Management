...
- **Current state**: The isolated Worker-side Etsy connection proof passed live for shop `PrintMOShirts` (`41261957`). Signed receipt ingestion, durable retry, bounded reconciliation, and automatic enrollment are live. On 2026-08-13, Etsy delivered real receipt `4144096237`; the Worker processed it once as `SHADOW_SYNCED`, reconciliation independently re-persisted it as `PAID_UNSHIPPED`, and the owner then authorized normal enrollment. Worker `e567ae65-1b16-4230-8d93-d9561be7f5dd` has `ETSY_WEBHOOK_ENROLLMENT_ENABLED=1`; the one eligible existing shadow receipt was promoted to `active` with `board_enrolled=1`.
- **Next safe action**: Refresh the board and accept the real card. Listing `4477292532` does not yet have a private catalog-preview mapping, so use the existing agent-operated import workflow if its color recognition image is desired.
- **Remaining blockers**: Canceled, refunded, shipped, additional multi-line, and other live variation/personalization edge cases remain unobserved. Supplier batching/artwork upload remain disabled for Etsy.
