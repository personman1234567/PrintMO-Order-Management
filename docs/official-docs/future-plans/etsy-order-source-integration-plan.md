...
- **Current state**: The isolated Worker-side Etsy connection proof passed live for shop `PrintMOShirts` (`41261957`). Signed receipt ingestion, durable retry, bounded reconciliation, and automatic enrollment are live. On 2026-08-13, Etsy delivered real receipt `4144096237`; the Worker processed it once as `SHADOW_SYNCED`, reconciliation independently re-persisted it as `PAID_UNSHIPPED`, and the owner then authorized normal enrollment. Worker `e567ae65-1b16-4230-8d93-d9561be7f5dd` has `ETSY_WEBHOOK_ENROLLMENT_ENABLED=1`; the one eligible existing shadow receipt was promoted to `active` with `board_enrolled=1`. Migration `0009` adds agent-managed preview-refresh decisions and exact listing-to-S&S blank recipes; its provider batch path keeps Etsy production state in D1 rather than Shopify metadata.
- **Next safe action**: Apply migration `0009` and deploy the Worker. Then, in agent chat, review any changed listing preview mapping one variant at a time and configure an exact supplier blank recipe for each Etsy listing before it enters S&S batching.
- **Remaining blockers**: Canceled, refunded, shipped, additional multi-line, and other live variation/personalization edge cases remain unobserved. A real Etsy listing must be explicitly configured to its actual supplier style; never infer Tultex 202 solely from a product title or color.
