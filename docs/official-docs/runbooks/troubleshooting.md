...
| Mobile board shows zero cards or **No pipeline orders match this view** while desktop works | The Shopify mobile app may be outdated, or a missing/late embedded context may have caused the queue request to fail while older dashboard behavior presented that failure as an empty result | `order-manager-web/index.html` embedded-context bootstrap; `web-shim.js` authentication/request boundary; `renderer.js:setBoardLoadState`; `dashboard-triage-enhancements.js:applyTriage` | Update the Shopify mobile app, fully close and reopen it, then launch PrintMO from Shopify Admin. Release `17876858473N` rendered the board correctly after the owner updated the app; the initial failure report was confounded by the stale native runtime. If an updated app still fails, record the visible load message and use **Try again** before investigating authentication. Preserve explicit loading/error state and show an empty pipeline only after a successful queue response. |
| Mobile Order Detail traps the operator or makes Orders, Blanks, Print, and Storage unresponsive | Detail was treated as a full-app modal: it covered the command surface and the inert isolation walk disabled everything outside the detail card | `mobile.css` Detail containment; `accessibility-hardening.js:isolateDialog`; `shopify-embedded-mobile.js:normalizeDetailPresentation`; `renderer.js:initMobileTabs` | Keep Detail inside `.app-content`, preserve the visible global command surface, expose the labeled **Back** action, and close Detail before navigating to another stage. Verify the covered board is inert, the command surface is not inert, and Back restores card focus at both 320px and 393px. Desktop Detail remains modal. |
...
| Previous Orders detail loads Designer Studio artwork but omits manually uploaded mockups | The read-only history capability correctly disables artwork writes, but shared Detail incorrectly used that write capability to suppress the separate manual-mockup read | `renderer.js:canReadManualMockupsForOrder`, `renderer.js:openDetail`, `web-shim.js:listManualMockups` | Keep manual-mockup reads provider-gated and independent of `artworkUpload`; history may read existing private Shopify mockups while upload/remove controls remain hidden. Verify the history predicate with Phase 2 before deploying Pages. |
...
| Studio mockups appear together only after a later automatic refresh | Private previews were fetched one-by-one into JavaScript blobs, or asset hydration did not repaint the affected column | `web-shim.js:prefetchCandidateAssetTickets`, `hydrateCandidateAssets`, `scheduleMockupRepaint` | Confirm the board makes one batch-ticket request, image elements stream signed reads directly, and the current column repaints as each mockup becomes ready. Manual R2 mockups remain a separate immediate path. |
| Old paid and fulfilled orders suddenly appear in Pipeline | A replayed `orders/updated` event or incremental reconciliation enrolled historical commerce records without a fulfillment/lifecycle gate | `webhookConfirmsPayment`, `orderCanEnterCandidateBoard`, `renderer.js:orderIsVisibleOnOperationalBoard` | Deploy the current Worker and Pages bundle. New enrollment must be paid, unfulfilled, non-cancelled, and open; already projected fulfilled/default-stage rows are excluded visually while deliberate `completed` production remains in Printed. |
| Shopify guest checkouts appear as `Name unavailable` even though Shopify Admin shows a recipient | Candidate summary lacks shipping/billing recipient name, or D1 still has the pre-fallback snapshot | `ORDER_SUMMARIES_QUERY`, `normalizeShopifySummary`, `handleV1OrdersGet`, `web-shim.js:candidateOrderToBoard` | Press **Refresh Shopify** once. The current projection falls back shipping → billing name and `refresh=1` bypasses the short TTL. If still absent, Shopify did not return an approved name field. |
...
| S&S submission reports a rejected garment or partial result | S&S returned a structured request or line error for a SKU, quantity, or inventory condition | `handleV1BatchCommit`, supplier gateway detailed response, S&S result dialog | Use the displayed batch reference and line feedback. Fully accepted orders may advance; affected orders remain in Build Order. Never resubmit an `unknown` result. |
...
- **Promotion behavior**: A purchased design may no longer exist at the preview key. Resolution first tries `orders/<orderNumber>/<designRef>/<file>`. The older customer-slug path retains a bounded five-page prefix/suffix lookup only as a legacy fallback. Zero matches remain unresolved; multiple legacy matches are rejected as ambiguous.
- **Canonical result**: Private `R2_BUCKET` bytes, matching source/destination SHA-256, an active `asset_manifests` blob row, and one or more `asset_manifest_links` associations. Repeated sizes or line items with identical bytes share the blob. Board responses expose one representative manifest ID/metadata, not `object_key`; the complete links load in order detail.
...
npx wrangler d1 execute printmo-order-manager --remote --command "SELECT m.order_gid,m.filename,l.role,l.side,l.line_item_id,l.design_ref,m.state FROM asset_manifests m LEFT JOIN asset_manifest_links l ON l.asset_id=m.id WHERE m.created_by='designer-studio-sync' ORDER BY m.order_gid,m.filename,l.line_item_id;"
```
...
The board intentionally requests only one representative asset per order and does not await artwork before rendering. One authenticated batch call issues opaque-ID, one-minute signed URLs; image elements then stream the files directly. On mobile, hidden workflow tabs do not request their previews until selected. The complete design/mockup set is ticketed only after order detail opens. A placeholder during either interval is expected and is not evidence that the order or manifest disappeared.
