...
| Etsy Seller App OAuth, receipt proof, field-shape inventory, hidden provider shadow, provider-aware order planning, or Etsy webhooks | [Etsy integration continuation](future-plans/etsy-order-source-integration-plan.md#current-continuation-state) | `worker.js → handleEtsyConnect, handleEtsyOauthCallback, handleEtsyTestRead, redactedJsonFieldShape, normalizeEtsyOrderContract, etsyShadowEligibility, handleEtsyShadowSync`; migrations `0004–0005` | `node --check order-manager-proxy/worker.js`; Phase 2; docs check | Importing a real eligible receipt, webhook subscription, write scopes, and automatic board enrollment are separately owner-gated |
