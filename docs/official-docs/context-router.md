...
| Etsy Seller App OAuth, receipt proof, provider shadow, synthetic/shared-board pilot, production state, or Etsy webhooks | [Etsy integration continuation](future-plans/etsy-order-source-integration-plan.md#current-continuation-state) | `worker.js → handleEtsyConnect, handleEtsyOauthCallback, handleEtsyShadowSync, handleEtsySyntheticOrder, handleProviderOrderDetailGet, handleProviderProductionPatch`; `web-shim.js → candidateOrderToBoard`; migrations `0004–0006` | `node --check order-manager-proxy/worker.js`; Phase 2; proxy tests/build; docs check | Real-receipt enrollment, webhook subscription, write scopes, automatic enrollment, and mixed-source supplier batches are separately owner-gated |
