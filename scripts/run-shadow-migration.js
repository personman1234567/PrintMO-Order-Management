...
async function run() {
  ...
  console.log(`Phase 2 migration ${opts.execute ? 'EXECUTE' : 'DRY RUN'}: ${opts.shop} (${opts.env})`);
  do {
    const response = await fetch(`${base}/order-manager/v1/migration/run`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ execute: opts.execute, confirmShop: opts.execute ? opts.confirmShop : undefined, offset, limit: opts.limit })
    });
